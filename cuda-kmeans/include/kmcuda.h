 ...
 * of the Classic K-Means with Consistent
 * Speedup"](https://www.microsoft.com/en-us/research/wp-content/uploads/2016/02/ding15.pdf). While it introduces
 * some overhead and many conditional clauses which are bad for CUDA, it still shows 1.6-2x speedup against the
 * Lloyd algorithm. K-nearest neighbors employ the same triangle inequality idea and require precalculated
 * centroids and cluster assignments, similar to the flattened ball tree.
 *
...
extern "C"
{
    ...
    /// @return KMCUDAResult.
    KMCUDAResult
    kmeans_cuda(KMCUDAInitMethod init, const void *init_params, float tolerance, float yinyang_t,
                KMCUDADistanceMetric metric, uint32_t samples_size, uint16_t features_size, uint32_t clusters_size,
                uint32_t seed, uint32_t device, int32_t device_ptrs, int32_t fp16x2, int32_t verbosity,
                const float *samples, float *centroids, uint32_t *assignments, float *average_distance);
    ...
    /// @return KMCUDAResult.
    KMCUDAResult
    knn_cuda(uint16_t k, KMCUDADistanceMetric metric, uint32_t samples_size, uint16_t features_size,
             uint32_t clusters_size, uint32_t device, int32_t device_ptrs, int32_t fp16x2, int32_t verbosity,
             const float *samples, const float *centroids, const uint32_t *assignments, uint32_t *neighbors);
...
const std::unordered_map<std::string, KMCUDAInitMethod> init_methods{{"kmeans++", kmcudaInitMethodPlusPlus},
...
const std::unordered_map<std::string, KMCUDADistanceMetric> metrics{
    {"euclidean", kmcudaDistanceMetricL2},  {"L2", kmcudaDistanceMetricL2},
    {"l2", kmcudaDistanceMetricL2},         {"cos", kmcudaDistanceMetricCosine},
...
const std::unordered_map<int, const char *> statuses{
    {kmcudaSuccess, "Success"},           {kmcudaInvalidArguments, "InvalidArguments"},
    {kmcudaNoSuchDevice, "NoSuchDevice"}, {kmcudaMemoryAllocationFailure, "MemoryAllocationFailure"},
    {kmcudaRuntimeError, "RuntimeError"}, {kmcudaMemoryCopyError, "MemoryCopyError"}};
} // namespace kmcuda
} // namespace
#endif // __cplusplus
