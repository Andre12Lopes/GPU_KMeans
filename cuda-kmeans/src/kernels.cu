...
calc_distance(const float *example, int nfeatures, float *cluster)
{
    float dist = 0.0;
    float corr = 0.0;
 ...
 */
__global__ void
normalize_clusters(float *clusters, const int *__restrict__ nmembers, const int nclusters, const int nfeatures)
{
 ...
 */
__global__ void
reset_clusters(float *clusters, int *nmembers, const int nclusters, const int nfeatures)
{
    int p_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (p_idx < nclusters * nfeatures)
    {
...
__global__ void
find_membership(const float *data, const int npoints, const int nfeatures, float *const clusters,
                const int nclusters, const int cchunk, int *assignments, int *assignments_prev, int *update)
{
    ...
    for (int cluster_offset = 0; cluster_offset < nclusters; cluster_offset += cchunk)
    {
        ...
        if (threadIdx.x < cchunk && cluster_offset + threadIdx.x < nclusters)
        {
            for (int i = 0; i < nfeatures; ++i)
            {
                s_clusters[(threadIdx.x * nfeatures) + i] =
                    clusters[(cluster_offset + threadIdx.x) * nfeatures + i];
            }
        }
        __syncthreads();
        ...
        if (p_idx < npoints)
        {
            ...
            for (int i = 0; i < cchunk && cluster_offset + i < nclusters; ++i)
            {
                ...
                float dist = calc_distance(&data[p_idx * nfeatures], nfeatures, &s_clusters[i * nfeatures]);
...
__global__ void
find_membership_global(const float *data, const int npoints, const int nfeatures, float *const clusters,
                       const int nclusters, int *assignments, int *assignments_prev, int *update)
{
    ...
    {
        float dist = calc_distance(&data[p_idx * nfeatures], nfeatures, &clusters[i * nfeatures]);
        if ((dist / min_dist) < limit)
        {
            min_dist = dist;
            ass = i;
 ...
 */
__global__ void
update_clusters_gmcl(const float *__restrict__ data, volatile float *clusters, volatile int *nmembers,
                     const int nclusters, const int npoints, const int nfeatures,
                     const int *__restrict__ assignments, const int *__restrict__ assignments_prev, int *locks)
{
    int p_idx = blockIdx.x * blockDim.x + threadIdx.x;
    volatile int done = 0;
    ...
    while (!done)
    {
        ...
        { // +1 needed so that p_idx=0 works
            for (int i = 0; i < nfeatures; ++i)
            {
 ...
 */
__global__ void
update_clusters_gmdl(const float *__restrict__ data, volatile float *clusters, volatile int *nmembers,
                     const int nclusters, const int npoints, const int nfeatures,
                     const int *__restrict__ assignments, const int *__restrict__ assignments_prev, int *locks)
{
    int p_idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void
update_clusters_smcl(const float *data, volatile float *clusters, volatile int *nmembers, const int nclusters,
                     const int cchunk, const int npoints, const int nfeatures, const int *assignments,
                     const int *assignments_prev, int *locks)
{
    ...
    volatile int *s_nmembers = reinterpret_cast<volatile int *>(s_mem + (cchunk * nfeatures));
            ...
            if (ass >= cluster_offset && ass < cluster_offset + cchunk)
            {
                ...
                while (!done)
                {
                    if (atomicCAS(&s_locks[s_target], 0, p_idx + 1) == 0)
                    {
                        ...
                        {
                            s_clusters[s_target * nfeatures + j] += data[p_idx * nfeatures + j];
                        }
                        done = 1;
        ...
        if (threadIdx.x < cchunk && target < nclusters)
        {
            ...
            while (!done)
            {
                if (atomicCAS(&locks[target], 0, threadIdx.x + 1) == 0)
                {
                    for (int i = 0; i < nfeatures; ++i)
                    {
                        clusters[(target)*nfeatures + i] += s_clusters[threadIdx.x * nfeatures + i];
                    }
                    done = 1;
...
//
__global__ void
update_clusters_smdl(const float *data, volatile float *clusters, volatile int *nmembers, const int nclusters,
                     const int cchunk, const int npoints, const int nfeatures, const int *assignments,
                     const int *assignments_prev, int *locks)
{
    int p_idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    // pointers to shared memory
    volatile float *s_clusters = reinterpret_cast<volatile float *>(s_mem);
    volatile int *s_nmembers = reinterpret_cast<volatile int *>(s_clusters + (cchunk * nfeatures));
    int *s_locks = const_cast<int *>(s_nmembers + cchunk);
...
/////////////////////////////////
__global__ void
update_clusters_gmct(const float *data, volatile float *clusters, volatile int *nmembers, const int nclusters,
                     const int npoints, const int nfeatures, const int *assignments, const int *assignments_prev,
                     int *locks)
{
    extern __shared__ float s_mem[];
...
__global__ void
update_clusters_gmdt(const float *data, volatile float *clusters, volatile int *nmembers, const int nclusters,
                     const int npoints, const int nfeatures, const int *assignments, const int *assignments_prev,
                     int *locks)
{
    extern __shared__ float s_mem[];
 ...
 * SM-CT
 */
// Simple shared memory implementation, using one thread for each cluster (mimicks Source{d} implementation,
// maintains STAMP update approach) [cluster-centric]
__global__ void
update_clusters_smct(const float *data, volatile float *clusters, volatile int *nmembers, const int nclusters,
                     const int cchunk, const int npoints, const int nfeatures, const int *assignments,
                     const int *assignments_prev, int *locks)
{
    extern __shared__ float s_mem[];
...
__global__ void
update_clusters_smdt(const float *data, volatile float *clusters, volatile int *nmembers, const int nclusters,
                     const int cchunk, const int npoints, const int nfeatures, const int *assignments,
                     const int *assignments_prev, int *locks)
{
    extern __shared__ float s_mem[];
...
__global__ void
update_clusters_shared_tb(const float *data, volatile float *clusters, volatile int *nmembers, const int nclusters,
...
//////////////////////////////////////
__global__ void
update_clusters_atomic(const float *__restrict__ data, float *clusters, int *nmembers, const int nclusters,
...
__global__ void
update_clusters_atomic_finegrain(const float *__restrict__ data, float *clusters, int *nmembers,
                                 const int nclusters, const int npoints, const int nfeatures,
                                 const int *__restrict__ assignments, const int *__restrict__ assignments_prev,
                                 int *locks)
{
    int p_idx = blockIdx.x * blockDim.x + threadIdx.x;
    volatile int done = 0;
...
__global__ void
update_clusters_scgcl(const float *data, volatile float *clusters, volatile int *nmembers, const int nclusters,
                      const int cchunk, const int npoints, const int nfeatures, const int *assignments,
                      const int *assignments_prev, int *locks)
{
...
update_clusters_scgdl(const float *__restrict__ data, volatile float *clusters, volatile int *nmembers,
                      const int nclusters, const int cchunk, const int npoints, const int nfeatures,
                      const int *assignments, const int *assignments_prev, int *locks)
{
    int p_idx = blockIdx.x * blockDim.x + threadIdx.x;
    extern __shared__ float s_mem[];
