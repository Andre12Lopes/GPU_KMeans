...
data_t::data_t(const int points, const int attrs) : numPoints(points), numAttrs(attrs)
{
...
void
data_t::setData(float **d, int p, int a)
{
    numPoints = p;
    numAttrs = a;
...
data_t *
extract_data(std::string filename)
{
...
void
printClusters(float *clusters, int nclusters, int nfeatures)
{
    std::cout << "[" << std::endl;
...
void
setRandomIndices(int *ptr, int len)
{
