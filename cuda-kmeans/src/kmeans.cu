...
void
setFindMembershipParameters(data_t *, int, int &, int &, int &, int &);
void
setUpdateParameters(int, data_t *, int, int &, int &, int &, int &);
void
setResetParameters(data_t *, int, int &, int &);
int
getNumBlocksNeeded(int, int);
std::string
getAlgoString(int);

int
cuda_kmeans(int algo, int k, data_t *d, int threads)
{
    // init clusters...
    float *h_data;
    float *h_clusters;
    ...
    gpuErrchk(cudaMemcpyAsync(data, h_data, sizeof(float) * d->numPoints * d->numAttrs, cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpyAsync(clusters, h_clusters, sizeof(float) * k * d->numAttrs, cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpyAsync(distances, h_distances, sizeof(float) * d->numPoints, cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpyAsync(assignments, h_assignments, sizeof(int) * d->numPoints, cudaMemcpyHostToDevice));
    gpuErrchk(
        cudaMemcpyAsync(assignments_prev, h_assignments_prev, sizeof(int) * d->numPoints, cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpyAsync(nmembers, h_nmembers, sizeof(int) * k, cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpyAsync(locks, h_locks, sizeof(int) * k, cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpyAsync(fg_locks, h_fg_locks, sizeof(int) * k * d->numAttrs, cudaMemcpyHostToDevice));
    ...
    do
    {
        ...
        *update = 0;
        ...
        //                 update);
        find_membership_global<<<m_blocks, m_threads>>>(data, d->numPoints, d->numAttrs, clusters, k, assignments,
                                                        assignments_prev, update);
        gpuErrchk(cudaDeviceSynchronize());

        int reassignments = 0;
        uint64_t iter_runtime = 0;
        gpuErrchk(cudaMemcpyAsync(h_assignments, assignments, sizeof(int) * d->numPoints, cudaMemcpyDeviceToHost));
        gpuErrchk(cudaMemcpyAsync(h_assignments_prev, assignments_prev, sizeof(int) * d->numPoints,
                                  cudaMemcpyDeviceToHost));

        gpuErrchk(cudaDeviceSynchronize());
        for (int i = 0; i < d->numPoints; ++i)
        {
            if (h_assignments[i] != h_assignments_prev[i])
                ++reassignments;
        }

        // check if any were reassigned
        done = true;
        if ((*update) / d->numPoints > 0.05)
        {
            done = false;
        }
        ...
        {
            // reset clusters
            reset_clusters<<<r_blocks, r_threads>>>(clusters, nmembers, k, d->numAttrs);
            gpuErrchk(cudaDeviceSynchronize());
            ...
            case GM_CL:
                ...
                update_clusters_gmcl<<<u_blocks, u_threads>>>(const_data, vol_clusters, vol_nmembers, k,
                                                              d->numPoints, d->numAttrs, const_ass, const_ass_prev,
                                                              locks);
                gpuErrchk(cudaDeviceSynchronize())
            ...
            case GM_DL:
                clock_gettime(CLOCK_REALTIME, &cstart);
                update_clusters_gmdl<<<u_blocks, u_threads>>>(const_data, vol_clusters, vol_nmembers, k,
                                                              d->numPoints, d->numAttrs, const_ass, const_ass_prev,
                                                              fg_locks);
                gpuErrchk(cudaDeviceSynchronize())
                    normalize_clusters<<<r_blocks, r_threads>>>(clusters, nmembers, k, d->numAttrs);
                gpuErrchk(cudaDeviceSynchronize()) clock_gettime(CLOCK_REALTIME, &cend);
                break;
            case SM_CL:
                clock_gettime(CLOCK_REALTIME, &cstart);
                update_clusters_smcl<<<u_blocks, u_threads, u_sharedmem>>>(const_data, clusters, nmembers, k,
                                                                           u_chunk, d->numPoints, d->numAttrs,
                                                                           const_ass, const_ass_prev, locks);
                gpuErrchk(cudaDeviceSynchronize())
                    normalize_clusters<<<r_blocks, r_threads>>>(clusters, nmembers, k, d->numAttrs);
                gpuErrchk(cudaDeviceSynchronize()) clock_gettime(CLOCK_REALTIME, &cend);
                break;
            case SM_DL:
                clock_gettime(CLOCK_REALTIME, &cstart);
                update_clusters_smdl<<<u_blocks, u_threads, u_sharedmem>>>(const_data, clusters, nmembers, k,
                                                                           u_chunk, d->numPoints, d->numAttrs,
                                                                           const_ass, const_ass_prev, locks);
                gpuErrchk(cudaDeviceSynchronize())
                    normalize_clusters<<<r_blocks, r_threads>>>(clusters, nmembers, k, d->numAttrs);
            ...
            case GM_CT:
                clock_gettime(CLOCK_REALTIME, &cstart);
                update_clusters_gmct<<<u_blocks, u_threads, u_sharedmem>>>(const_data, vol_clusters, vol_nmembers,
                                                                           k, d->numPoints, d->numAttrs, const_ass,
            ...
            case GM_DT:
                clock_gettime(CLOCK_REALTIME, &cstart);
                update_clusters_gmdt<<<u_blocks, u_threads, u_sharedmem>>>(const_data, vol_clusters, vol_nmembers,
                                                                           k, d->numPoints, d->numAttrs, const_ass,
                                                                           const_ass_prev, locks);
                gpuErrchk(cudaDeviceSynchronize())
                    normalize_clusters<<<r_blocks, r_threads>>>(clusters, nmembers, k, d->numAttrs);
            ...
            case SM_CT:
                clock_gettime(CLOCK_REALTIME, &cstart);
                update_clusters_smct<<<u_blocks, u_threads, u_sharedmem>>>(const_data, clusters, nmembers, k,
                                                                           u_chunk, d->numPoints, d->numAttrs,
                                                                           const_ass, const_ass_prev, locks);
                gpuErrchk(cudaDeviceSynchronize())
                    normalize_clusters<<<r_blocks, r_threads>>>(clusters, nmembers, k, d->numAttrs);
            ...
            case SM_DT:
                clock_gettime(CLOCK_REALTIME, &cstart);
                update_clusters_smdt<<<u_blocks, u_threads, u_sharedmem>>>(const_data, clusters, nmembers, k,
                                                                           u_chunk, d->numPoints, d->numAttrs,
                                                                           const_ass, const_ass_prev, locks);
                gpuErrchk(cudaDeviceSynchronize())
                    normalize_clusters<<<r_blocks, r_threads>>>(clusters, nmembers, k, d->numAttrs);
            ...
            case SC_GCL:
                clock_gettime(CLOCK_REALTIME, &cstart);
                update_clusters_scgcl<<<u_blocks, u_threads, u_sharedmem>>>(const_data, clusters, nmembers, k,
                                                                            u_chunk, d->numPoints, d->numAttrs,
                                                                            const_ass, const_ass_prev, locks);
                gpuErrchk(cudaDeviceSynchronize()) clock_gettime(CLOCK_REALTIME, &cend);
                break;
            case SC_GDL:
                clock_gettime(CLOCK_REALTIME, &cstart);
                update_clusters_scgdl<<<u_blocks, u_threads, u_sharedmem>>>(const_data, clusters, nmembers, k,
                                                                            u_chunk, d->numPoints, d->numAttrs,
                                                                            const_ass, const_ass_prev, fg_locks);
                gpuErrchk(cudaDeviceSynchronize()) clock_gettime(CLOCK_REALTIME, &cend);
                break;
            }
        ...
        gpuErrchk(cudaDeviceSynchronize()) for (int i = 0; i < k; ++i)
        {
    ...
    std::printf("%s\t%d\t%f\t%LF\t%d\t%d\t%d\t%d\n", algoString.c_str(), k, tot_rt, updt_rt, loop, u_threads,
                u_blocks, u_sharedmem);
    return 1;
}

void
setFindMembershipParameters(data_t *d, int k, int &m_threads, int &m_blocks, int &m_chunk, int &m_sharedmem)
{
    m_threads = M_THREADS;
    m_blocks = (d->numPoints % m_threads == 0 ? d->numPoints / m_threads : d->numPoints / m_threads + 1);
    m_chunk = MAX_SHMEM / (sizeof(float) * d->numAttrs);
    m_sharedmem = sizeof(float) * (d->numAttrs * m_chunk);
}

void
setUpdateParameters(int algo, data_t *d, int k, int &u_threads, int &u_blocks, int &u_chunk, int &u_sharedmem)
{
    int need;
    switch (algo)
    {
    ...
    case SM_DL: // SM-DL
        need = d->numPoints * d->numAttrs;
        u_blocks = getNumBlocksNeeded(need, u_threads);
        u_chunk = MAX_SHMEM / (sizeof(float) * d->numAttrs + sizeof(int) + sizeof(int) * d->numAttrs);
        u_chunk = (u_chunk > k ? k : u_chunk);
        u_sharedmem =
            sizeof(float) * u_chunk * d->numAttrs + sizeof(int) * u_chunk + sizeof(int) * u_chunk * d->numAttrs;
        break;
    case GM_CT: // GM-CT
        need = k;
        u_blocks = getNumBlocksNeeded(need, u_threads);
...
void
setResetParameters(data_t *d, int k, int &r_threads, int &r_blocks)
{
    r_threads = R_THREADS;
