...
void
test_kmcuda(data_t *d, int k, int s)
{
    ...
    printf("%d\t%d\t", 0, k);
    KMCUDAResult results =
        kmeans_cuda(init, nullptr, tolerance, 0.0, L2, samples_size, features_size, clusters_size, seed, device,
                    device_ptrs, fp16x2, verbosity, samples, centroids, assignments, nullptr);
    printf("NA\tNA\tNA\n");
}
